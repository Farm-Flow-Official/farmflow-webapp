...
/** ≥0.70 ok / 0.50–0.69 watch / <0.50 critical. */
function confClasses(c: number): string {
  if (c >= 0.7) return 'bg-success-bg text-success'
  if (c >= 0.5) return 'bg-warning-bg text-warning'
  return 'bg-error-bg text-error'
}

/** Deterministic earthy gradient for the photo placeholder (no real image yet). */
        ...
        return (
          ...
          >
            ...
            <div
              ...
              style={placeholderStyle(t.id)}
            >
              <TreePine className="h-8 w-8 text-white/40" strokeWidth={1.5} />
              <span
                className={`absolute right-1.5 top-1.5 rounded px-1.5 py-0.5 font-mono text-[11px] font-bold ${confClasses(t.aiConfidenceScore)}`}
              >
                {pct}%
              </span>
              {t.anomaly && (
